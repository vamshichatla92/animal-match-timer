...
import AnimalEach from '../AnimalEach'
...
const AnimalMatch = () => {
    ...
    const selectTabActiveAnimal = (tabId) => {
      setGameState(prev => ({ ...prev, category: tabId }));
    };
  
    const selectedImage = (thumbnailUrl) => {
      const imageValue = imagesList.find(eachValue => eachValue.thumbnailUrl === thumbnailUrl);
  
      if (imageValue.imageUrl === gameState.imageB) {
        const randomImage = imagesList[Math.floor(Math.random() * imagesList.length)].imageUrl;
        setGameState(prev => ({
          ...prev,
    ...
    const filteredImagesList = imagesList.filter(eachX => eachX.category === gameState.category);
      ...
      <div>
        {!gameState.gameOver ? (
          <>
            ...
            <img src={gameState.imageB} className="imgg" alt="jio" />
  
            <ul className="adj-A">
              {tabsList.map(eachTab => (
                <TabItem
                  activeAnimal={selectTabActiveAnimal}
                  key={eachTab.tabId}
                  tabDetails={eachTab}
                />
              ))}
          ...
          <span>Game Over! Your score: {gameState.points}</span>
        )}
      </div>
    );
  };
